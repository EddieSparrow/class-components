...
export default function Films() {
  ...
  function handleCheckbox(newItem: string, event: EventTarget) {
    ...
    if (isChecked) {
      ...
      dispatch(setItems(items.filter((item) => item != newItem)));
      dispatch(setCount(count - 1));
    }
    console.log([...items]);
    console.log(count);
    ...
    <div className="results">
      {filmList.data.movie_count === 0 ? (
        ...
        filmList.data.movies?.map((film: Movies) => (
          <div>
            <input className="film-checkbox" type="checkbox" onChange={(event) => handleCheckbox(film.id, event.target)} checked={items.some((item) => item === film.id)} />
